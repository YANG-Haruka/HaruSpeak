"""Japanese language module — binds per-file implementations into the Protocol."""
...
# Import functions directly — the submodule names are shadowed by
# `__init__.py`'s re-exports of same-named functions.
...
class JapaneseModule(LanguageModule):
    ...
    async def tts_reference(self, text: str) -> bytes:
        ...
        return await edge_synthesize(text, lang="ja", voice=settings.tts_voice_ja)
